-- ==============================================================================
-- TourneyMaster AI - Master Database Schema & Evolution Migration
...
-- Auto-create/link profile on signup & designate superadmin for guillermoriveraterriza@gmail.com
...
CREATE TABLE IF NOT EXISTS public.tournament_config (
  ...
  mystery_mode_duration_seconds INTEGER NOT NULL DEFAULT 900,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.tournament_groups (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT tournament_groups_tournament_category_group_key UNIQUE (tournament_id, category, group_code)
);
...
CREATE TABLE IF NOT EXISTS public.player_aliases (
  ...
  resolution_status TEXT NOT NULL DEFAULT 'confirmed' CHECK (resolution_status IN ('pending', 'confirmed', 'rejected')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT unique_alias_source UNIQUE (alias, source_system)
);

CREATE INDEX IF NOT EXISTS idx_player_aliases_normalized ON public.player_aliases(normalized_alias);

-- ============================================================
-- 10. HISTORICAL IMPORTS, TOURNAMENTS, GROUPS, MATCHES
-- ============================================================
...
CREATE TRIGGER update_rating_states_updated_at
  BEFORE UPDATE ON public.rating_states
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TABLE IF NOT EXISTS public.rating_snapshots (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT unique_player_tournament_snapshot UNIQUE (player_id, historical_tournament_id)
);
...
BEGIN
  ...
  IF p_confirming_user_id <> v_match.player1_id AND p_confirming_user_id <> v_match.player2_id THEN
    IF NOT public.is_admin() THEN
      RETURN jsonb_build_object('success', false, 'error', 'Only match participants or admins can confirm');
    END IF;
  END IF;

  IF p_confirming_user_id = v_match.reported_by AND NOT public.is_admin() THEN
...
-- Tournament Participants: viewable by all, users can join, admins can manage
...
CREATE POLICY "Service role full access on players" ON public.players FOR ALL USING (true);
CREATE POLICY "Service role full access on player_aliases" ON public.player_aliases FOR ALL USING (true);
CREATE POLICY "Service role full access on historical_tournaments" ON public.historical_tournaments FOR ALL USING (true);
CREATE POLICY "Service role full access on historical_groups" ON public.historical_groups FOR ALL USING (true);
CREATE POLICY "Service role full access on historical_matches" ON public.historical_matches FOR ALL USING (true);
CREATE POLICY "Service role full access on rating_states" ON public.rating_states FOR ALL USING (true);
CREATE POLICY "Service role full access on rating_snapshots" ON public.rating_snapshots FOR ALL USING (true);
CREATE POLICY "Service role full access on historical_imports" ON public.historical_imports FOR ALL USING (true);
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'tournament_groups'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'match_reports'
  ) THEN
